// Agent classification per spec: OS class is immutable at runtime
export type AgentClass = "os" | "planner" | "action";

// Review decisions per ReviewInterface spec
...
export interface LieutenantPlanResult {
  plan: Plan;
  missingScripts: { name: string; capability: string }[];
}
...
export interface AgentConfig {
  ...
  class: AgentClass;
  configFile: string;
  agentMd: string; // contents of XYZ-AGENT.md
  permissions: {
    canRead: string[];
    canWrite: string[];
  };
...
export interface IdentityContext {
  ...
  constitution: string; // CONSTITUTION.md contents
  agents: AgentConfig[];
...
export interface AgentOutput {
  content: string;
  thought?: string; // Brief user-facing summary of what the agent is doing and why
  usage: {
    inputTokens: number;
    outputTokens: number;
  };
