...
async function main(): Promise<void> {
  ...
  if (restored.length > 0) {
    ...
    const answer = await new Promise<string>((res) => {
      ...
      tempRl.question(
        `Previous session found (${turnCount} turn${turnCount === 1 ? "" : "s"}). Resume? (y/n): `,
        (ans) => {
          tempRl.close();
  ...
  adapter.onRequest(async (input) => {
    if (input === "/clear") {
      conversationHistory = [];
      saveSession(SESSIONS_DIR, []).catch((err) =>
        adapter.sendError(`[sessions] Failed to clear session: ${err}`)
      );
    ...
    try {
      ...
      saveSession(SESSIONS_DIR, conversationHistory).catch((err) =>
      ...
      if (!skipReview) {
        sampleAndAudit(client, identity, adapter, LOGS_DIR, 1.0, {
          ...
          verbose("RR: audit error (non-blocking)", err instanceof Error ? err.message : String(err))
        );
      }
    } catch (err) {
