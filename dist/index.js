...
async function appendExecutionLog(input, scriptResults) {
    await mkdir(LOGS_DIR, { recursive: true });
...
async function main() {
    ...
    adapter.onRequest(async (input) => {
        ...
        try {
            const result = await handleRequest(client, input, identity, SCRIPTS_DIR, PLANS_DIR, conversationHistory, skipReview, adapter);
