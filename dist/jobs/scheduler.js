...
export function createScheduler(store, executor, adapter, options = {}) {
    ...
    async function processCallbacks(job) {
        const triggerEvent = job.status === "completed" ? "complete" : "fail";
        for (const cb of job.callbacks) {
            if (cb.on !== triggerEvent)
                continue;
            try {
                if (cb.action === "create_job" && cb.payload) {
                    await store.createJob({
                        type: cb.payload.type ?? "execute_script",
                        goal: cb.payload.goal ?? "Callback job",
                        dependsOn: cb.payload.dependsOn ?? [],
                        createdBy: job.id,
                        evidence: [],
                        callbacks: [],
                        channel: job.channel,
    ...
    async function blockDependents(failedId) {
