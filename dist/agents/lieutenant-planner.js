 ...
 */
export async function createDetailedPlan(client, assignment, identity, scriptsDir, plansDir, scriptSubset) {
 ...
 */
export function detectMissingScripts(plan) {
    return plan.steps
 ...
 */
export async function createDetailedPlanWithDW(client, assignment, identity, scriptsDir, plansDir, options = {}) {
    ...
    while (result.missingScripts.length > 0 && dwCallCount < MAX_DW_CALLS) {
        ...
        for (const missing of toCreate) {
            try {
                await generateAndPromote(client, { capability: missing.capability, existingScripts }, identity, scriptsDir, { adapter: options.adapter, onPromote: options.onPromote });
                dwCallCount++;
...
function formatDetailedPlanMd(plan, assignment) {
    const lines = [
