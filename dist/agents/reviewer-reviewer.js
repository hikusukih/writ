 ...
 */
function buildRRPrompt(identity) {
 ...
 */
function buildAuditMessage(entry) {
 ...
 */
export async function auditReviewDecision(client, input, identity) {
 ...
 */
async function appendRRAuditLog(entry, result, logsDir) {
 ...
 */
export async function sampleAndAudit(client, identity, adapter, logsDir, sampleRate, options = {}) {
    ...
    if (!result.consistent) {
        ...
        if (options.identityDir && options.runtimeDir) {
            ...
            triggerBigBrother(violation, client, identity, adapter, options.identityDir, options.runtimeDir).catch((err) => verbose("BB: trigger error (non-blocking)", err instanceof Error ? err.message : String(err)));
        }
    }
    return result;
}
