 ...
 */
export async function generateScript(client, request, identity) {
...
{"scriptName": "my-script", "scriptContent": "#!/bin/bash\\n# @name my-script\\n...", "testSuggestions": "optional"}`;
    ...
    const result = await callWithValidation(client, systemPrompt, userMessage, DeveloperWriterResultSchema, { label: "DeveloperWriter" });
 ...
 */
export async function stageScript(content, scriptName) {
 ...
 */
export async function promoteScript(stagingPath, scriptsDir, options) {
 ...
 */
export async function generateAndPromote(client, request, identity, scriptsDir, options) {
