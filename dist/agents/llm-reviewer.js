...
function buildReviewerPrompt(identity, reviewerConfig, antiPatterns) {
    const parts = [
 ...
 */
export async function reviewWithLLM(content, subjectAgentId, identity, client, reviewerConfig, antiPatterns, logsDir) {
...
Does this output violate any security constraints, ethical guidelines, or the system's soul/constitution above? Return your JSON decision.`;
    try {
        ...
        console.error(`[reviewer] LLM review failed, falling back to rule-based: ${err instanceof Error ? err.message : String(err)}`);
