 ...
 */
export function shouldSemanticReview(config) {
    if (!config.enabled)
        return false;
 ...
 */
export async function semanticReview(client, composedScript, planDescription, identity) {
    const systemPrompt = [
...
Does this script faithfully implement the plan? Return your JSON assessment.`;
    ...
    const result = await callWithValidation(client, systemPrompt, userMessage, SemanticReviewResultSchema, { label: "Semantic review" });
