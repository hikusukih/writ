 ...
 */
function buildBBPrompt(identity) {
 ...
 */
export async function proposeConfigUpdate(client, input, identity) {
 ...
 */
export async function applyConfigUpdate(client, bbOutput, violation, identity, adapter, identityDir, runtimeDir, options = {}) {
 ...
 */
async function selfModLoop(client, input, identity, adapter, identityDir, runtimeDir) {
    for (let round = 1; round <= MAX_SELF_MOD_ROUNDS; round++) {
        ...
        const applied = await applyConfigUpdate(client, output, input.violation, identity, adapter, identityDir, runtimeDir);
        if (applied) {
    ...
    if (adapter) {
        adapter.sendError(`BIG_BROTHER failed to produce an approved config update after ${MAX_SELF_MOD_ROUNDS} attempts ` +
 ...
 */
export async function triggerBigBrother(violation, client, identity, adapter, identityDir, runtimeDir) {
